...
/*   Updated: 2024/06/19 16:00:07 by dloisel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_map_parser(t_game *game)
{
	...
	ft_count_map_parameters(game, 0, 0);
...
void	ft_count_map_parameters(t_game *game, int i, int j)
{
	i = -1;
	while (++i < game->map.rows)
	{
		...
		while (++j < game->map.columns)
		{
			if (game->map.fullmap[i][j] == PLAYER)
			{
				...
				game->map.exit_count++;
			else if (game->map.fullmap[i][j] == ENEMY)
